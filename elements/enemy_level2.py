...
#---- FEATURE NUEVOS ENEMIGOS - NIVEL 2 ----
class EnemyLevel2(pygame.sprite.Sprite):
    def __init__(self, screen, tipo):
        ...
        if tipo == "normal":
            ...
            imagen = pygame.image.load("assets/bug_S.png").convert_alpha()
            tamaño = (48, 48)
            self.speed = 8
            self.vidas = 1

        elif tipo == "G":
            imagen = pygame.image.load("assets/bug_G.png").convert_alpha()
            tamaño = (90, 90)
            self.speed = 2
            self.vidas = 5

        elif tipo == "M":
            imagen = pygame.image.load("assets/bug_M.png").convert_alpha()
            tamaño = (70, 70)
            self.speed = 3
            self.vidas = 1

        elif tipo == "D":
            imagen = pygame.image.load("assets/bug_D.png").convert_alpha()
            tamaño = (75, 75)
            self.speed = 4
            self.vidas = 1

        self.image = pygame.transform.scale(imagen, tamaño)

        if tipo == "D":
            self.rect = self.image.get_rect(
                center=(
                    self.screen_width - 100,
                    random.randint(80, self.screen_height - 80)
                )
            )
            ...
            self.rect = self.image.get_rect(
                center=(
                    self.screen_width + 100,
                    random.randint(self.image.get_height() // 2, self.screen_height - self.image.get_height() // 2)
                )
            )
    ...
    def update(self, player_pos, proyectiles):
        ...
        #---- BUG D - MOVIMIENTO ALEATORIO Y REBOTE ----
        if self.tipo == "D":
